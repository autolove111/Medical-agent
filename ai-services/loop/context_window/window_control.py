...
class ContextWindowControl:
    ...
    def print_summary(self) -> None:
        ...
        logger.info("=" * 50)
        logger.info("总窗口: %d tokens", self.total_tokens)
        logger.info("输出预留: %d tokens", int(self.total_tokens * self.output_ratio))
        logger.info("可用: %d tokens", self.available)
        logger.info("=" * 50)
        for name, l in self._limits.items():
            pct = l.tokens / self.available * 100
            logger.info("  %12s: %6d tokens (%4.1f%%) = %5d chars", name, l.tokens, pct, l.chars)
        logger.info("  %12s: %6d tokens", "合计", total_used)
        logger.info("=" * 50)
