...
<!-- 修复：使用 Composition API，与其他组件保持一致 -->
<script setup>
import { ref } from "vue";
...
async function handleLogin() {
  ...
  // 修复：添加身份证号格式校验
  if (!validateIdNumber(loginForm.value.idNumber)) {
    errorMessage.value = "请输入有效的18位身份证号";
    return;
  }
...
async function handleRegister() {
  ...
  if (registerForm.value.age === null || registerForm.value.age === "") {
    errorMessage.value = "年龄不能为空";
    return;
  }

  // 修复：添加身份证号格式校验
  if (!validateIdNumber(registerForm.value.idNumber)) {
    errorMessage.value = "请输入有效的18位身份证号";
    return;
  }
...
function toggleForm() {
  ...
  errorMessage.value = "";
  loginForm.value = { idNumber: "", password: "" };
  registerForm.value = {
    realName: "",
    idNumber: "",
    age: null,
