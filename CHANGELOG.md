...
| 文件 | 改动 |
|------|------|
| \python_service/.env\ | \RAG_LOCAL_EMBEDDING_PATH\ 从 \ce-embedding-base_v1\ 改为 \Zhinao-ChineseModernBert-Embedding\ |
...
- **保留兼容**：\i-services-python/ocr_service/\ 目录保留，\eport.py\ 路由层不改
...
| \python_service/service/rag/hybrid_retriever.py\ | 改 | FAISS 召回 5→20、\erank_and_truncate()\ 替换为 CrossEncoder 语义打分 |
...
| 模块 | 测试脚本 | 结果 |
|------|----------|------|
| 嵌入模型 | \	est_rag/test_rag_e2e.py\ | ✅ 768 维编码正常 |
| 重排器 | \	est_rag/test_rag_e2e.py\ | ✅ 语义排序精确，阈值过滤有效 |
| 端到端 | \	est_rag/test_rag_e2e.py\ | ✅ 5 条医学查询全部通过 |
| OCR | \_test15.py\（已清理） | ⚠️ API 流程打通，但 token 权限需确认 |
