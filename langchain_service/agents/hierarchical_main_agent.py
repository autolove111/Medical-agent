...
from graph.builder import run_consultation_workflow
from .coordinator import DepartmentAgentCoordinator, ConsensusResult
from models.weight_updater import get_weight_updater
from knowledge.medical_knowledge import create_knowledge_base
from knowledge.reference_ranges import get_reference_range
from tools import query_medical_knowledge, query_user_age_profile, query_user_medical_history
...
# 主Agent核心方法：计算注意力权重、识别关键指标簇、执行主循环、更新权重、生成最终诊断、处理用户反馈等
    ...
    async def analyze_lab_results(
        ...
        """分析化验结果，内部通过 LangGraph 状态图编排多轮会诊流程。"""
        return await run_consultation_workflow(
            agent=self,
